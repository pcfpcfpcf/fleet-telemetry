    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
        ...
        def heartbeat() -> None:
            while True:
                ...
                with _stats_lock:
                    print(
                        f"[demo-server] heartbeat connections={_connections} packets={_packets}",
                        flush=True,
                    )
        ...
        while True:
            ...
            def client_worker(client_conn: socket.socket, client_addr: tuple[str, int]) -> None:
                with client_conn:
                    try:
                        handle_client(client_conn, client_addr)
                    except Exception as exc:
                        print(f"[demo-server] client {client_addr} disconnected: {type(exc).__name__}: {exc}")
